...
fn embed_data(img: &DynamicImage, data: &[u8]) -> Result<RgbaImage, SteganoError> {
...
fn extract_data(img: &DynamicImage) -> Result<Vec<u8>, SteganoError> {
    let (width, height) = img.dimensions();
...
async fn encode_message(
    ...
    let output_img = embed_data(&img, &encrypted_data)?;
...
async fn decode_message(image_base64: String, passphrase: String) -> Result<String, SteganoError> {
    ...
    let encrypted_data = extract_data(&img)?;
